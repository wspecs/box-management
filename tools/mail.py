	...
	try:
		...
		if e.code == 401:
			try:
				...
				pass
			print("The management daemon refused access. The API key file may be out of sync. Try 'service wspecsbox restart'.", file=sys.stderr)
		elif hasattr(e, 'read'):
...
def setup_key_auth(mgmt_uri):
	key = open('/var/lib/wspecsbox/api.key').read().strip()

	auth_handler = urllib.request.HTTPBasicAuthHandler()
	auth_handler.add_password(
		realm='wspecsbox Management Server',
		uri=mgmt_uri,
		user=key,
		passwd='')
